...
int8_t Accelerometer_ADXL375_OLD::begin() {
...
int8_t Accelerometer_ADXL375_OLD::readData() {
    uint8_t buffer[6];
    memset(buffer, 0, sizeof buffer);

    readMultipleRegisters(buffer, 0x32, 6);

    int32_t x = ((int16_t) buffer[0]) | (((int16_t) buffer[1]) << 8);
    int32_t y = ((int16_t) buffer[2]) | (((int16_t) buffer[3]) << 8);
    int32_t z = ((int16_t) buffer[4]) | (((int16_t) buffer[5]) << 8);
